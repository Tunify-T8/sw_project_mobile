...
void main() {
  group('Upload coverage buffer - library uploads provider', () {
    ...
    test('refresh stores a friendly error and resets refreshing state on failure', () async {
      when(
    ...
    test('deleteTrack stores a friendly error and clears busy state on failure', () async {
      when(
      ...
      container.read(libraryUploadsProvider.notifier).state =
          LibraryUploadsState(
    ...
    test('replaceFile stores a friendly error and clears busy state on failure', () async {
      when(
    ...
    test('updateTrack refreshes uploads and clears busy state on success', () async {
  ...
  group('Upload coverage buffer - simple upload units', () {
    test('CloudinaryUploadConfig exposes usable values', () {
      expect(CloudinaryUploadConfig.cloudName, isNotEmpty);
      expect(CloudinaryUploadConfig.audioUploadPreset, isNotEmpty);
      expect(CloudinaryUploadConfig.imageUploadPreset, isNotEmpty);
      expect(CloudinaryUploadConfig.apiKey, isA<String>());
      expect(CloudinaryUploadConfig.apiSecret, isA<String>());
    });
    ...
    test('GlobalTrackStore can clear one owner without affecting another owner', () {
      ...
      final second = sampleUploadItem.copyWith(id: 'track-2', title: 'Second');

      GlobalTrackStore.instance.add(first, ownerUserId: 'user-1');
      GlobalTrackStore.instance.add(second, ownerUserId: 'user-2');

      expect(GlobalTrackStore.instance.ownerUserIdForTrack('track-1'), 'user-1');
      expect(GlobalTrackStore.instance.ownerUserIdForTrack('track-2'), 'user-2');

      GlobalTrackStore.instance.clear(ownerUserId: 'user-1');

      expect(GlobalTrackStore.instance.allForUser('user-1'), isEmpty);
      expect(GlobalTrackStore.instance.ownerUserIdForTrack('track-1'), isNull);
      expect(
    ...
    test('mockUploadServiceProvider builds the upload mock service', () {
      ...
      expect(container.read(mockUploadServiceProvider), isA<MockUploadService>());
    });
  });
}